...
@client.event
async def on_ready():
    #runs when the code is activated
...
async def on_member_join(member):
    guild = client.get_guild(914756125583343638)
    ...
    if author.find('testing_bot') == -1: #if the one sending a message is a real user
        ...
        try:
            ...
            elif len(active_conversations[id]) == 3:
                active_conversations[id].append(str(message.content))
...
client.run('OTE0NDQzOTYxMjk1MzMxMzg4.YaNITQ.RKHV62MNGUptbjkwbiIj8Vn_ZeY')  #token from Discord bot